...
class LineMessageParser:
    ...
    # メタメッセージ（除外対象）のパターン
    # iPhone版とAndroid版の両方に対応（☎記号の有無、通話種別のバリエーション）
    META_PATTERNS = [
        r"^\[スタンプ\]$",
        r"^\[写真\]$",
        r"^\[動画\]$",
        ...
        r"がメンバーを追加しました。$",
        r"がメンバーを削除しました。$",
        r"^☎?\s?不在着信$",  # ☎は任意（iPhone/Android対応）
        r"^☎?\s?通話に応答がありませんでした。?$",  # ☎と末尾の。は任意
        r"^☎?\s?通話時間 \d+:\d+$",  # ☎は任意
        r"^☎?\s?通話をキャンセルしました。?$",  # ☎と末尾の。は任意
        r"^☎?\s?グループ(音声|ビデオ)?通話が開始されました。$",  # 音声/ビデオは任意、☎も任意
        r"^☎?\s?グループ通話が終了しました。$",  # ☎は任意
    ]
