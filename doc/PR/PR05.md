...
1. **`__init__(min_length: int = 1, exclude_parts: set[str] | None = None)`**
   - MeCabタガーの初期化
   - **引数**
     - `min_length`: 最小単語長（デフォルト: 1）
     - `exclude_parts`: 除外する品詞のセット（デフォルト: None）
...
#### TestMorphologicalAnalyzerクラス（17テスト）
...
11. `test_exclude_specific_pos`: 特定の品詞を除外
12. `test_exclude_multiple_pos`: 複数の品詞を除外
13. `test_filter_non_independent_noun`: 非自立名詞の除外
14. `test_filter_pronoun`: 代名詞の除外

**品詞抽出テスト**
15. `test_extract_interjection`: 感動詞の抽出

**基本形抽出テスト**
16. `test_base_form_extraction`: 基本形の正確な抽出

**エラーハンドリングテスト**
17. `test_mecab_initialization_error`: MeCab初期化エラーのハンドリング
